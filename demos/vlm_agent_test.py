...
@click.option("--question", default="You are entering a Pokemon battle. What is a list of the pokemon and levels on the enemy trainer's team?", type=str)
@click.option("--visual_context", default="You are in a Pokemon battle screen. And Lass is declaring she wants to fight.", type=str)
...
def do(model_name, init_state, game_variant, question, visual_context, max_steps):
    ...
    environment = get_environment(game=game_variant, environment_variant="default", controller_variant="state_wise", 
                                        save_video=True, max_steps=max_steps,
                                            init_state=init_state, session_name=f"vlm_test_demo_{short_model}", headless=True)
    vl = EQASupervisor(game=game_variant, environment=environment, 
                          executor_class=EQAPokemonExecutor, execution_report_class=EQAPokemonExecutionReport,
