...
class PokemonExecutor(SimpleExecutor):
    ...
    def get_action_str(self, *, allowed_actions_str, prev_action_strings = [], system_prompt = ""):
        ...
        if state == AgentState.FREE_ROAM:
            ...
            if len(prev_actions) > 0 and len(prev_action_strings) == 0: # Don't keep retrying this
                (action_string, action_class, action_kwargs, transition_states, success_code, action_return_info, action_message) = prev_actions[-1]
                if action_class == InteractAction and success_code == -1:
                    check_interaction_return, check_interaction_success, check_interaction_message = self.run_executor_action("checkinteraction()")
    ...
    def _string_to_executor_action(self, action_str):
        ...
        if action_str == "checkinteraction()" or action_str.startswith("checkinteraction("):
    ...
    def _decide_next_action_str(self, prev_action_strings = []):
        ...
        for action_class, action_desc in allowed_actions.items():
            allowed_actions_str += f"- {action_desc}\n"
        state = self._environment.get_info()["pokemon_core"]["agent_state"]
        if state == AgentState.FREE_ROAM and len(prev_action_strings) == 0: # don't allow locate retries because its expensive to run.
        ...
        action_str = self.get_action_str(allowed_actions_str=allowed_actions_str, prev_action_strings=prev_action_strings)
        if state == AgentState.FREE_ROAM and "locate" in action_str: # then run it here
            action_return, action_success, action_message = self.run_executor_action(action_str)
            if action_return is None: # error, can send it back and rety
                return action_str
            if "item" in action_str or "pokeball" in action_str:
                action_str = action_str + " (note, in pokemon all items on the map are shown as a pokeball icon)"
            ...
            if action_success == -1:
                ...
                for direction in ["up", "down", "left", "right"]:
                    if action_return[direction][1] == True:
                        found_directions[direction] = f"In the direction {direction}, there is {action_return[direction][0]}. To interact with this, first use the action `move({direction} 1)` and then use `interact()` again."
